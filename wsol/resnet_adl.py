...
class ResNetAcol(AcolBase):
    ...
    def forward(self, x, labels=None, return_cam=False):
        ...
        logits_dict = self._acol_logits(feature=feature, labels=labels,
                                        drop_threshold=self.drop_threshold, return_cam=return_cam)
        ...
        if return_cam:
            ...
            b, h, w = normalized_a.shape
            cams = normalized_a + normalized_b
            #cams = torch.max(normalized_a, normalized_b)
            #cams = feature_map[range(batch_size), labels]
            return cams.view(b,h,w), logits_dict['logits']
        ...
        if return_cam:
            ...
            for i in range(b):
             ...
             #cm_neg = cams_neg[i]
              ...
              maxi_pos = cm_pos.max()
              maxi_pos = maxi_pos * 0.25 #0.25 is best now
              idx_pos = ((cm_pos > maxi_pos) + 0)
              cams_pos[i] = cams_pos[i] * idx_pos.view(w, d)

              #cm_neg = cm_neg.view(-1)
              #maxi_neg = cm_neg.max()
              #maxi_neg1 = maxi_neg * 0.7
              #idx_neg1 = ((cm_neg < maxi_neg1) + 0)
              #maxi_neg2 = maxi_neg * 0.3
              #idx_neg2 = ((cm_neg > maxi_neg2) + 0)
              #cams_neg[i] = cams_neg[i] * idx_neg1.view(28, 28)
              #cams_neg[i] = cams_neg[i] * idx_neg2.view(28, 28)
