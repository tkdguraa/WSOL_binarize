...
class ResNetCam(nn.Module):
    ...
    def forward(self, x, labels=None, return_cam=False):
        ...
        if return_cam:
            ...
            #maximum, _ = torch.max(feature_map, dim=-1, keepdim=True)
            #cam_weights, idx = cam_weights.topk(2048, -1)
            #total = []
            #for i in range(b):
            #    lis_pos = []
            #    maps = feature_map[i]
            ...
            #    cam_sorted = torch.cat(lis_pos, 0).view(1, -1, h, w)
            #    total.append(cam_sorted)
            #cams_channel = torch.cat(total, 0)
